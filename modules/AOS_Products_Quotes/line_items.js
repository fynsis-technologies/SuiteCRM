...
/**
 * Insert productbundle line
 */
 
function insertProductbundleLine(tableid, groupid) {
    ...
    sqs_objects["product_name[" + prodln + "]"] = {
        "form": "EditView",
        "method": "query",
        "modules": ["pb_productbundles"],
        "group": "or",
        "field_list": ["name", "id","partnumber_c", "cost_c", "price_c","description","currency_id"],
        "populate_list": ["product_name[" + prodln + "]", "product_product_id[" + prodln + "]", "product_part_number[" + prodln + "]", "product_product_cost_price[" + prodln + "]", "product_product_list_price[" + prodln + "]", "product_item_description[" + prodln + "]", "product_currency[" + prodln + "]"],
        "required_list": ["product_id[" + prodln + "]"],
        "conditions": [{
            "name": "name",
    ...
    sqs_objects["product_part_number[" + prodln + "]"] = {
        ...
        "modules": ["pb_productbundles"],
        "group": "or",
        "field_list": ["partnumber_c", "name", "id","cost_c", "price_c","description","currency_id"],
        "populate_list": ["product_part_number[" + prodln + "]", "product_name[" + prodln + "]", "product_product_id[" + prodln + "]",  "product_product_cost_price[" + prodln + "]", "product_product_list_price[" + prodln + "]", "product_item_description[" + prodln + "]", "product_currency[" + prodln + "]"],
        "required_list": ["product_id[" + prodln + "]"],
        "conditions": [{
            "name": "partnumber_c",
...
var line_product_quantity =document.getElementById('product_product_qty' + prodln);
    ...
    var b2 = x.insertCell(3);
    b2.innerHTML = "<button title='" + SUGAR.language.get('app_strings', 'LBL_SELECT_BUTTON_TITLE') + "' accessKey='" + SUGAR.language.get('app_strings', 'LBL_SELECT_BUTTON_KEY') + "' type='button' tabindex='116' class='button' value='" + SUGAR.language.get('app_strings', 'LBL_SELECT_BUTTON_LABEL') + "' name='btn1' onclick='openProductbundlePopup(" + prodln + ");'><img src='themes/default/images/id-ff-select.png' alt='" + SUGAR.language.get('app_strings', 'LBL_SELECT_BUTTON_LABEL') + "'></button>";
...
/**
 * Open product bundle popup
 */
function openProductbundlePopup(ln){
	
	lineno=ln;
	var popupRequestData = {
		"call_back_function" : "setProductbundleReturn",
		"form_name" : "EditView",
		"field_to_name_array" : {
			"id" : "product_product_id" + ln,
			"name" : "product_name" + ln,
            "description" : "product_item_description" + ln,
            "partnumber_c" : "product_part_number" + ln,
			"cost_c" : "product_product_cost_price" + ln,
			"price_c" : "product_product_list_price" + ln,
            "currency_c" : "product_currency" + ln
		}
	};

	open_popup('pb_productbundles', 800, 850, '', true, true, popupRequestData);

}

function setProductbundleReturn(popupReplyData){
...
/**
 * Insert product Bundle Header
 */
 
function insertProductbundleHeader(tableid){
	tablehead = document.createElement("thead");
	tablehead.id = tableid +"_head";
	tablehead.style.display="none";
	document.getElementById(tableid).appendChild(tablehead);
...
var productbundleTableHeader = document.createElement("thead");
	table.appendChild(productbundleTableHeader);
	var productbundleHeader_row=productbundleTableHeader.insertRow(-1);
	var productbundleHeader_cell = productbundleHeader_row.insertCell(0);
	productbundleHeader_cell.colSpan="100";
	var productbundleTable = document.createElement("table");
	productbundleTable.id = "product_bundle_group"+groupn;
	productbundleHeader_cell.appendChild(productbundleTable);

	insertProductbundleHeader(productbundleTable.id);
...
footer_cell.innerHTML+="<input type='Button' value='Add Product Bundle' onclick='insertProductbundleLine(\""+productbundleTable.id+"\",\""+groupn+"\")'>";
...
function applyVolumeBasedDiscounts(prodln)
{
   
   if(document.getElementById( 'product_product_qty' + prodln) != null && document.getElementById( 'product_product_qty' +prodln).value == ''){
        document.getElementById( 'product_product_qty' +prodln).value =1;
   }
        
     var line_product_quantity  = unformat2Number(document.getElementById('product_product_qty' + prodln).value);
     var line_id=document.getElementById('product_product_id'+prodln).value;
  ...
  $.ajax({
        ...
        {
        	product_id:line_id,
            product_quantity: line_product_quantity            
        },
        type: "POST",
        dataType: "json",
        success: function(data)
        {
                 ...
                 document.getElementById('product_product_discount' + prodln).value = data.discount;
         
            if (data.discount_type=="Percentage"){
                 document.getElementById('product_discount' + prodln).selectedIndex="0";
            }
            if(data.discount_type=="Amount")
            {
                 document.getElementById('product_discount' + prodln).selectedIndex="1";
            }
